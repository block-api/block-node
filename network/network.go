...
type Manager struct {
	...
	cStopSenderWorker   chan int
	cStopReceiverWorker chan int
	cSend               chan Packet
	cReceive            chan Packet
	cResponse           chan Packet
...
func NewManager(nodeID string, nodeName string, nodeVersion int, config *params.NetworkConfig, functionManager *function.Manager) (*Manager, error) {
	if manager == nil {
		...
		manager = &Manager{
			...
			cStopSenderWorker:   make(chan int),
			cStopReceiverWorker: make(chan int),
			cSend:               make(chan Packet, 10),
			cReceive:            make(chan Packet),
			cResponse:           make(chan Packet, 10),
...
func (m *Manager) Start() error {
	...
	go m.receiverWorker(m.transport, m.cStopReceiverWorker, m.cReceive, m.wgStop)
...
func (m *Manager) Stop() error {
	...
	m.cStopSenderWorker <- 1
	m.cStopReceiverWorker <- 1
